...
class TrainDataLoader(object):
    ...
    def collate_fn(batch):
        feats, labels, labels_time, frame_mask_list = generate_feats_labels(batch)
        return BatchInfo(feats, labels, labels_time, frame_mask_list)
...
def generate_feats_labels(batch):
    ...
    feat_list = torch.view_as_real(feat_list).permute(0,3,2,1) #B F T C-> B C T F
    label_list = torch.stft(label_list, n_fft=fft_num, hop_length=win_shift, win_length=win_size,
                            window=torch.hann_window(fft_num),return_complex=True)
    label_list = torch.view_as_real(label_list).permute(0,3,2,1) 
    return feat_list, label_list, label_list_time, frame_mask_list
...
def cv_generate_feats_labels(batch):
    ...
    feat_list = torch.stft(feat_list, n_fft=fft_num, hop_length=win_shift, win_length=win_size,
                           window=torch.hann_window(fft_num),return_complex=True)
    feat_list = torch.view_as_real(feat_list).permute(0,3,2,1)
    label_list = torch.stft(label_list, n_fft=fft_num, hop_length=win_shift, win_length=win_size,
                            window=torch.hann_window(fft_num),return_complex=True)
    label_list = torch.view_as_real(label_list).permute(0,3,2,1)
    return feat_list, label_list, label_list_time, frame_mask_list
...
class CvDataLoader(object):
    ...
    @staticmethod
    def collate_fn(batch):
        feats, labels, labels_time, frame_mask_list = cv_generate_feats_labels(batch)
        return BatchInfo(feats, labels, labels_time, frame_mask_list)
...
class BatchInfo(object):
    def __init__(self, feats, labels, labels_time, frame_mask_list):
        self.feats = feats
        self.labels = labels
