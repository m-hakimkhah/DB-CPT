...
class Solver(object):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.having == True:
                ...
                for i in range(len(optim_state['param_groups'])):
                    optim_state['param_groups'][i]['lr'] = optim_state['param_groups'][i]['lr'] / 2
    ...
    def run_one_epoch(self, epoch, cross_valid=False):
        def _batch(_, batch_info):
            ...
            batch_label_time = batch_info.labels_time
            ...
            if not cross_valid:
                batch_loss = com_mag_mse_loss(esti_list, batch_label, batch_label_time, batch_frame_mask_list)
                ...
                if is_pesq:
                    batch_loss_res = pesq_loss(esti_list, batch_label_time, batch_frame_mask_list)
                else:
                    batch_loss = com_mag_mse_loss(esti_list, batch_label, batch_label_time, batch_frame_mask_list)
                    batch_loss_res = batch_loss.item()
                tr_batch.append(batch_loss_res)
