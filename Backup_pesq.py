...
def com_mag_mse_loss(esti, label, label_time, frame_list):
    utt_num = esti.size()[0]
    with torch.no_grad():
    ...
    loss4 = []
    for id in range(utt_num):
      loss4.append(time_loss_SDR(id, esti_utts, clean_utts))
    loss4 = np.mean(loss4)

    return 0.5*loss1 + 0.6*loss2 + 0.2*loss3 + 0.004*loss4


def pesq_loss(esti_list, label_time, frame_list):
    with torch.no_grad():
...
def time_loss_SDR(id, esti_utts, clean_utts):
    clean_utt = clean_utts[id]
    esti_utt = esti_utts[id]
    sdr = 10*np.log10(np.sum(esti_utt**2)/np.sum((esti_utt-clean_utt)**2))
